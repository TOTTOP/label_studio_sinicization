...
def debug_settings_view(request):
    # 检查 DEBUG 模式
    debug_mode = f"settings.DEBUG: {settings.DEBUG} (类型: {type(settings.DEBUG).__name__})"

    # 检查 STATIC_URL
    static_url = f"settings.STATIC_URL: '{settings.STATIC_URL}'"

    # 检查 STATICFILES_DIRS
    staticfiles_dirs_str = "settings.STATICFILES_DIRS:\n"
    for i, path in enumerate(settings.STATICFILES_DIRS):
        exists = "存在 (exists)" if os.path.exists(path) else "不存在 (DOES NOT EXIST)"
        staticfiles_dirs_str += f"  [{i}]: {path}  --> {exists}\n"
    
    # 检查 INSTALLED_APPS
    staticfiles_app_installed = "'django.contrib.staticfiles' in settings.INSTALLED_APPS: {'django.contrib.staticfiles' in settings.INSTALLED_APPS}"

    # 检查 ROOT_URLCONF
    root_urlconf = f"settings.ROOT_URLCONF: '{settings.ROOT_URLCONF}'"

    # 组合所有信息
    response_html = f"""
    <pre>
    <h1>Django 运行时配置诊断</h1>
    <hr>
    <h2>核心配置</h2>
    {debug_mode}
    <br>
    {staticfiles_app_installed}
    <br>
    {root_urlconf}
